...
import { useGetAllRABs, useGetAllPondoks } from '@/hooks/use-yayasan-data';
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Kelola RAB dari semua Pondok"
      />
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative w-full md:w-1/2">
              ...
              <Input
                placeholder="Cari RAB berdasarkan nama pondok atau periode..."
                className="pl-8"
                value={searchTerm}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              Tidak ada RAB yang {activeTab === 'diajukan' ? 'diajukan' : activeTab === 'revisi' ? 'perlu direvisi' : 'diterima'}
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {filteredRABs?.map((rab: any) => (
                  <TableRow key={rab.id}>
                    <TableCell className="font-medium">{rab.pondok?.nama || 'Unknown Pondok'}</TableCell>
                    <TableCell>{rab.periode_id.substring(0, 4)}/{rab.periode_id.substring(4, 6)}</TableCell>
                    <TableCell>{formatDate(rab.submit_at)}</TableCell>
