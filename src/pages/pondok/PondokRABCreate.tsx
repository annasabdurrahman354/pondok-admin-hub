...
const PondokRABCreate: React.FC = () => {
  ...
  const { data: rabs = [], isLoading: isRabsLoading } = useGetRABs();
  const { createRABMutation } = useRABMutations();
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      <PageHeader 
        ...
        className="mb-2"
      >
        <Button variant="outline" size="sm" onClick={() => navigate('/pondok/rab-list')}>
          <ChevronLeft className="mr-2 h-4 w-4" />
          Batal
        </Button>
      </PageHeader>
...
export default PondokRABCreate;
